...
public:
    ...
    ESockaddrIn(UINT nIP, short nPort) {
        ...
        //m_addr.sin_addr.s_addr = htonl(nIP); // 过时了
        InetPton(AF_INET, _T("127.0.0.1"), &m_addr.sin_addr); // _T宏处理字符编码
        m_ip = inet_ntoa(m_addr.sin_addr);
        m_port = nPort;
    }
...
public:
    ...
    int connect(const std::string& ip, short port) {
        ESockaddrIn addr(ip, port);
        return ::connect(m_socket, addr, sizeof(addr));
    }
    ...
    int sendto(const EBuffer& buffer, const ESockaddrIn& to) {
        return ::sendto(m_socket, buffer, buffer.size(), 0, to, sizeof(to));
    }
