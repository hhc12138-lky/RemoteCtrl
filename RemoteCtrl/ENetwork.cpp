...
EServer::EServer(const EServerParameter& param) :m_stop(false)
{
	m_params = param;
...
{
    int ret = m_sock->send(buffer);  // 发送TCP数据（需处理不完整发送情况）
    if (m_params.m_send) m_params.m_send(m_args, client, ret);  // 触发发送回调
    return ret;
}

int EServer::Sendto(ESockaddrIn& addr, const EBuffer& buffer)
{
    int ret = m_sock->sendto(buffer, addr);  // 发送UDP数据（需处理不完整发送情况）
    if (m_params.m_sendto) m_params.m_sendto(m_args, addr, ret);  // 触发发送回调
    return ret;
}
